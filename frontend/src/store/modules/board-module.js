...
export default {
      ...
      if (
        ...
        for (var i = 0; i < state.currBoard.groups.length; i++) {
          ...
          if (taskFiltered.length > 0) {
            if (
              !copyCurrBoard.groups.find((group) => group.id === groupRun.id)
            ) {
  ...
  actions: {
    ...
    async removeGroup(
      { dispatch, commit },
      { boardId, groupId, removedIndex = null }
    ) {
      try {
    ...
    async addItem(
      { dispatch, commit },
      { boardId, groupId, task, fromIdx = null }
    ) {
      try {
    ...
    async updateGroup(
      { dispatch, commit },
      { boardId, currGroup, addedIdxForDrop = null }
    ) {
